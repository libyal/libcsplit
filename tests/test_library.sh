...
LIBRARY_TESTS="error narrow_split_string narrow_string support wide_split_string wide_string";
...
run_test()
{
	...
	local TEST_EXECUTABLE="./csplit_test_${TEST_NAME}";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libcsplit" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
